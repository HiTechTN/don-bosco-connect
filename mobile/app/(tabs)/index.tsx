        ...
        <>
          ...
          {analytics && (
            <View style={styles.card}>
              ...
              <Text style={styles.stat}>Leçons: {analytics.completed_lessons}</Text>
              <Text style={styles.stat}>Score: {analytics.average_score?.toFixed(1)}%</Text>
            </View>
          )}
        </>
      )}
...
const styles = StyleSheet.create({
  ...
  welcome: { fontSize: 24, fontWeight: 'bold', color: '#1a365d', marginBottom: 16 },
  card: { backgroundColor: 'white', padding: 16, borderRadius: 12, marginBottom: 16 },
  cardTitle: { fontSize: 18, fontWeight: '600', color: '#1a365d', marginBottom: 12 },
  stat: { fontSize: 16, color: '#4a5568', marginBottom: 8 },
});