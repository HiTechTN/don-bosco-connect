...
- Name: public-portal-ar.spec.ts >> ContactPage — Arabic rendering >> shows Arabic submit button
- Location: tests/public-portal-ar.spec.ts:314:3
...
Locator: locator('button').filter({ hasText: 'إرسال الرسالة' })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('button').filter({ hasText: 'إرسال الرسالة' })
